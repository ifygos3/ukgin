        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-800">
              ...
              <th className="text-left p-3 text-gray-400">Page Slug</th>
              <th className="text-left p-3 text-gray-400">Page Type</th>
              <th className="text-left p-3 text-gray-400">Published</th>
          ...
          <tbody>
            {pages.map((page) => (
              <tr key={page.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                <td className="p-3 text-gray-400">{page.id}</td>
                <td className="p-3 text-white font-bold">{page.title}</td>
                <td className="p-3 text-gray-400">{page.slug}</td>
                <td className="p-3 text-gray-400">{page.page_type}</td>
                <td className="p-3">{page.is_published ? <span className="bg-green-500/20 text-green-300 px-2 py-1 rounded text-xs">Yes</span> : <span className="bg-gray-500/20 text-gray-300 px-2 py-1 rounded text-xs">No</span>}</td>
                <td className="p-3 text-gray-400 text-xs">{new Date(page.updated_at).toLocaleDateString()}</td>
                <td className="p-3">
                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => openEdit(page)} className="text-blue-400 hover:text-blue-300 text-xs font-bold">Edit</button>
                    <button onClick={() => handleDelete(page.id)} className="text-red-400 hover:text-red-300 text-xs font-bold">Delete</button>
                  </div>
                </td>
              </tr>
            ))}
